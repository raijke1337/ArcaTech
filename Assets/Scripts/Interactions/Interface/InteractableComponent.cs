...
public class InteractableComponent : ValidatedMonoBehaviour
{
    ...
    [Header("Post Effects")]
    [SerializeField] private List<InteractionEffect> successEffects;
    ...
    /// </summary>
    private IEnumerator ExecuteBlockingEffects(List<InteractionEffect> effects, InteractionContext ctx, InteractionState stageState)
    {
    ...
    public void CancelInteraction()
    {
        ...
        if (preExecuteEffects != null)
            foreach (var e in preExecuteEffects) e?.OnCancelled();
        if (duringExecuteEffects != null)
            foreach (var e in duringExecuteEffects) e?.OnCancelled();
